...
void loop() {
  ...
  if (water_level == HIGH) {
    ...
    if ((millis() > last_watering_end + watering_interval_min) && (moisture_perc <= 30)) {
      ...
      while (moisture_perc <= 30) {
        ...
        if(millis() > last_watering_start + watering_max_time){
          ...
          myBot.sendMessage(chatID, (String)"Plants were wattered during "+((last_watering_end-last_watering_start)/60000)+" minutes but they are still thirsty. Try to set watering_max_time to a higher value");
          break;
        }
    ...
    } else if (moisture_perc <= 30) {
      myBot.sendMessage(chatID, (String)"Plants are thirsty but were wattered "+((millis()-last_watering_end)/3600000)+" hours ago. Try to set watering_interval_min to a lower value");
    }
       
    if ((moisture_perc <= 30) && (millis() > last_watering_start + watering_max_time)) {
      // ...and if the soil is dry, turn on the water pump (only if the total watering time is less than watering_max_time)
      digitalWrite(relay, HIGH);
    } else {
